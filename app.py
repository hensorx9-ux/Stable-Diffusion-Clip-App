...
# Get HF Token from Streamlit Secrets (for cloud) or .env / Environment (for local)
...
if not HF_TOKEN:
    st.warning("⚠️ Hugging Face token not found. Please add HF_TOKEN to your .env or Streamlit Secrets.")
...
if st.sidebar.button("Generate & Evaluate"):
    if not HF_TOKEN:
        ...
        with st.spinner("Generating images via Hugging Face Inference API..."):
            client = InferenceClient(api_key=HF_TOKEN)
            prompts = [prompt_1, prompt_2]
            
            # Generate images remotely via HF API
            images = []
            for p in prompts:
                img = client.text_to_image(p, model="CompVis/stable-diffusion-v1-4")
                images.append(img)

            # Evaluate with local lightweight CLIP model
